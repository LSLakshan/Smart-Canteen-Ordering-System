  ...
  useEffect(() => {
    const fetchBreakfast = async () => {
      try {
        ...
        if (response.ok && data.breakfast) {
          setBreakfastItems(data.breakfast);
        } else {
          setBreakfastItems([]);
        }
      } catch (err) {
        setError("Failed to fetch breakfast items.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const saveOrderToBackend = async (token) => {
    try {
      const orderData = {
        ...
        token: token,
        notes: `Breakfast order - ${selectedItem.name}`,
      };
    ...
    <div className="min-h-screen bg-yellow-50 p-6">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <h1 className="text-3xl font-bold text-yellow-800">Breakfast Menu</h1>
        </div>

        {loading ? (
          <p className="text-lg text-gray-700 text-center">
            Loading breakfast items...
          </p>
        ) : error ? (
            ...
            <div className="bg-white rounded-lg p-6 w-96 max-w-md mx-4">
              ...
              <div className="mb-6">
                ...
                <div className="space-y-2">
                  {["6:30 AM", "6:45 AM", "7:00 AM", "7:15 AM", "7:30 AM"].map(
                    (timeSlot) => (
                      <label
                        key={timeSlot}
